...
from envs.haptix import rooms_camera_env
...
# model_paths
TEMPLATE_PATH = 'rooms/template_camera.xml'
...
# task class
class CameraEnv(rooms_camera_env.RoomsEnv, utils.EzPickle):
    def __init__(
        self,
        #model_path,
    ...
    def _reset_sim(self):
        ...
        for _ in range(10):
            ...
            action = np.zeros(self.n_actuators + self.n_positions) #self.init_mpos
            #if self.control_hand_position or self.control_camera_position:
            if self.control_camera_position:
                action[self.n_actuators:] = self.init_mpos.reshape(self.n_positions,)[:]
    ...
    def init(self, info=None):
        ...
        with tempfile.NamedTemporaryFile(suffix=".xml", dir=os.path.join(prefix_path, 'rooms')) as fp:
            ...
            # init environment
            rooms_camera_env.RoomsEnv.__init__(
                self,
                modified_model_path,
...
class RoomsCameraMultiObjsEnv(CameraEnv):
    def __init__(self):
        super().__init__(
...
class RoomsCameraSingleObjEnv(CameraEnv):
    def __init__(self):
        super().__init__(
            #control_hand_position=True,
            control_camera_position=True,
