...
#from envs.haptix import rotations, toy_camera_env
from envs.haptix import toy_camera_env
#from envs.haptix.utils import robot_get_obs
...
# model_paths
TEMPLATE_PATH = 'toy/template_camera.xml'
MODEL_PATHS = {
        'sphere': 'toy/sphere.xml',
        'box': 'toy/box.xml',
        'cylinder': 'toy/cylinder.xml',
        #'ellipsoid': 'toy/ellipsoid.xml',
        }
#MODEL_PATH = 'toy/box.xml'
ROOM_PATH = 'toy/room.xml'

SCALE = 0.035


def estimate_size(scale, obj_type):
    if obj_type == 'sphere':  # 4/3*pi*r^3
        return scale
    elif obj_type == 'box':  # d^3
        return math.pow(4/3/8*math.pi, 1/3) * scale
    elif obj_type == 'cylinder': # pi*r^2*h = 2*pi*r^3 (h = 2r)
        return math.pow(2/3, 1/3) * scale

# gen_toy_3d_obj generation func
#def gen_toy_3d_obj(obj_type, scale=1., do_random_rotation=False):
#    # check condition
...
# task class
class ManipulateEnv(toy_camera_env.ToyEnv, utils.EzPickle):
    def __init__(
        self,
        #model_path,
        ...
        """Initializes a new Toy manipulation environment.
        ...
        self.configs['obj_type']=obj_type
    ...
    def _get_obs(self):
        return np.array([], dtype=np.float32) #self.sim.data.sensordata.copy()
    ...
    def _gen_scene_xml(self, full_path, obj_elt, table_elt=None):
        ''' get model '''
        # load and parse xml
    ...
    def _get_obj_info_elt(self, full_path, obj_type='sphere', rand_pos=False):
        ''' init info '''
        info = {}
        ...
        ''' set size '''
        size = estimate_size(scale, obj_type)
        info['size'] = size
        ...
        ''' generate gen_toy_3d_obj (as a set of points) '''
        # copy element
        curr_elt = deepcopy(element)

        # update name
        #curr_elt.set('name', 'obj-{}'.format(obj_type))
        curr_elt.set('name', 'obj'.format(obj_type))

        # update position
        curr_elt.set('pos', '{:.5f} {:.5f} {:.5f}'.format(pos[0], pos[1], pos[2]))

        ## update material
        #geom = curr_elt.findall(".//geom[@type='{}']".format(obj_type))[0]
        ...
        # update size
        geom = curr_elt.findall(".//geom[@type='{}']".format(obj_type))[0]
        geom.set('size', '{:.5f} {:.5f} {:.5f}'.format(size, size, size))

        return info, curr_elt
    ...
    def init(self, info=None):
        ...
        with tempfile.NamedTemporaryFile(suffix=".xml", dir=os.path.join(prefix_path, 'toy')) as fp:
            ...
            # init environment
            toy_camera_env.ToyEnv.__init__(
                self,
                modified_model_path,
...
#class ToyCameraMultiObjsEnv(ManipulateEnv):
#    def __init__(self):
#        super().__init__(
...
class ToyCameraSingleObjSphereEnv(ManipulateEnv):
    def __init__(self):
        super().__init__(
            #control_hand_position=True,
            control_camera_position=True,
...
class ToyCameraSingleObjBoxEnv(ManipulateEnv):
    def __init__(self):
        super().__init__(
            #control_hand_position=True,
            control_camera_position=True,
...
class ToyCameraSingleObjCylinderEnv(ManipulateEnv):
    def __init__(self):
        super().__init__(
            #control_hand_position=True,
            control_camera_position=True,
