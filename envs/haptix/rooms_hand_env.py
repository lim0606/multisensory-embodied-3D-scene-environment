...
class RoomsEnv(haptix_env.HaptixEnv):
    ...
    def _set_action(self, action):
        #''' split action '''
        #action_actuators = action[:self.n_actuators]  # actuators: 0-12 <- for MPL hand
        #action_positions = action[self.n_actuators:]  # pos: 13-15 <- for MPL hand
        ...
        if self.control_hand_position:
            ...
            # set angle
            angle = np.pi + set_angle(action[-1])  # pos: 13-15 <- for MPL hand
